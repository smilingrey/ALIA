<script type="text/javascript">
	...
	function Kirim_Cari()
	{
		...
		var s;
		s='./index.php?mod=home&opt=ho&opts=list';
...
<div id="judul_content"><h2>DATA IZIN GANGGUAN</h2></div>
...
<?php
	...
	if(!empty($_GET["nama"]) || !empty($_GET["nim"])) 
	{
		$qsrt = mysql_query("select * from tbl_ho where id_ho is not null and ".$up."");
	}
	else
	{
		$qsrt = mysql_query("select * from tbl_ho");
	}	
	$batas = 20;
	...
	if(!empty($_GET["nama"]) || !empty($_GET["nim"])) 
	{
		$qsrt = mysql_query("select * from tbl_ho where id_ho is not null and ".$up." order by no_sk limit ".$posisi.",".$batas."");
	}
	else
	{
		$qsrt = mysql_query("select * from tbl_ho order by no_sk limit ".$posisi.",".$batas."");
	}
	...
	echo '</span></form>';
?>
	<div><a href="./index.php?mod=home&opt=ho&opts=tambah" class="link" title="Klik untuk tambah"><button>TAMBAH DATA IZIN GANGGUAN</button></a></div><br />
    <table cellpadding="3" cellspacing="1" align="center" id="tbl" width="100%">
    <tr id="tbl-label">
        <td width="5%">NO</td>
        <td align="left">NOMOR IZIN</td>
...
?>
	<tr id="tbl-content">
        ...
        <td align="center" valign="top"><a class="edit" href="./index.php?mod=home&opt=ho&opts=edit&id_ho=<?php echo $bsrt["id_ho"]; ?>"><button>EDIT</button></a></td>
        <td align="center" valign="top"><a class="hapus" href="./index.php?mod=home&opt=ho&opts=validasi&mode=3&id_ho=<?php echo $bsrt["id_ho"]; ?>" onClick="return confirm('Apakah Anda yakin ingin menghapus data ini?' + '\nNomor Surat: ' + '<?php echo $bsrt["no_sk"]; ?>' + '\n\n' + 'Jika YA silahkan klik OK, Jika TIDAK klik CANCEL.')"><button>HAPUS</button></a></td>
    </tr>
<?php echo
    "<tr id='tbl-content'>

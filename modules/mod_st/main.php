<script type="text/javascript">
	function tampil(c,uid)
	{
			...
			if(t.innerHTML != ''){
				...
				t.innerHTML='<iframe src="./modules/mod_st/tampil.php?link='+c+'&id_user='+uid+'" width="100%" style="height:400px" frameborder="0" scrolling="no" id="iframe_detail"></iframe>';
	...
	function Kirim_Cari()
	{
		...
		var s;
		s='./index.php?mod=home&opt=st&opts=list';
...
<?php
	...
	if(!empty($_GET["nama"]) || !empty($_GET["nim"])) 
	{
		$qsrt = mysql_query("select * from tbl_tanah where id_tanah is not null and ".$up."");
	}
	else
	{
		$qsrt = mysql_query("select * from tbl_tanah");
	}	
	$batas = 20;
	...
	if(!empty($_GET["nama"]) || !empty($_GET["nim"])) 
	{
		$qsrt = mysql_query("select * from tbl_tanah where id_tanah is not null and ".$up." order by no_sk limit ".$posisi.",".$batas."");
	}
	else
	{
		$qsrt = mysql_query("select * from tbl_tanah order by no_sk limit ".$posisi.",".$batas."");
	}
	...
	echo '</span></form>';
?>
	<div><a href="./index.php?mod=home&opt=st&opts=tambah" class="link" title="Klik untuk tambah"><button>TAMBAH DATA SURAT TANAH</button></a></div><br />
...
?>
	<tr id="tbl-content">
        ...
        <td align="center" valign="top"><a class="edit" href="./index.php?mod=home&opt=st&opts=edit&id_tanah=<?php echo $bsrt["id_tanah"]; ?>"><button>EDIT</button></a></td>
        <td align="center" valign="top"><a class="hapus" href="./index.php?mod=home&opt=st&opts=validasi&mode=3&id_tanah=<?php echo $bsrt["id_tanah"]; ?>" onClick="return confirm('Apakah Anda yakin ingin menghapus data ini?' + '\nNomor Surat: ' + '<?php echo $bsrt["no_sk"]; ?>' + '\n\n' + 'Jika YA silahkan klik OK, Jika TIDAK klik CANCEL.')"><button>HAPUS</button></a></td>
    </tr>
<?php echo
    "<tr id='tbl-content'>

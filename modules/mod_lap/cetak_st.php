    ...
    <body>
        <table width="100%" cellspacing="0" cellpadding="0" class="ttd">
            <tr>
                ...
                <td align="center" class="judul">LAPORAN PENDATAAN SURAT KETERANGAN TANAH<br>
												<?php echo skpd(); ?><br>
												<?php echo pemerintah(); ?>
                </td>
            </tr>
...
<?php
	if(!empty($_GET["nama"]) || !empty($_GET["nim"]) || !empty($_GET["tgl1"]) || !empty($_GET["tgl2"]))
	{
		$qsrt = mysql_query("select * from tbl_tanah where id_tanah is not null and ".$up." order by no_sk ");
	}
	else
	{
		$qsrt = mysql_query("select * from tbl_tanah order by no_sk ");
	}
?>				

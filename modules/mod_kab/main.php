<script type="text/javascript">
	function Kirim_Cari()
	{
		...
		var s;
		s='./index.php?mod=home&opt=kab&opts=list';
...
<?php
	...
	if(!empty($_GET["nama"])) 
	{
		$qsrt = mysql_query("select * from ref_kabupaten where id_wilayah is not null  and ".$up."");
	}
	else
	{
		$qsrt = mysql_query("select * from ref_kabupaten ");
	}	
	$batas = 20;
	...
	if(!empty($_GET["nama"])) 
	{
		$qsrt = mysql_query("select * from ref_kabupaten where id_wilayah is not null  and ".$up." order by id_prov, wilayah limit ".$posisi.",".$batas."");
	}
	else
	{
		$qsrt = mysql_query("select * from ref_kabupaten order by id_prov, wilayah limit ".$posisi.",".$batas."");
	}
	...
	echo '</span></form>';
?>
	<div><a href="./index.php?mod=home&opt=kab&opts=tambah" class="link" title="Klik untuk tambah"><button>TAMBAH KABUPATEN</button></a></div><br />
...
<?php
     ...
     {
		$prov=$bsrt["id_prov"];
		$nmpri=mysql_query("select wilayah from ref_provinsi where id_wilayah='".$prov."'");
...
?>
	<tr id="tbl-content">
        ...
        <td align="center" valign="top"><a class="edit" href="./index.php?mod=home&opt=kab&opts=edit&id_prov=<?php echo $bsrt["id_wilayah"]; ?>"><button>EDIT</button></a></td>
        <td align="center" valign="top"><a class="hapus" href="./index.php?mod=home&opt=kab&opts=validasi&mode=3&id_prov=<?php echo $bsrt["id_wilayah"]; ?>" onClick="return confirm('Apakah Anda yakin ingin menghapus data ini?' + '\nNama: ' + '<?php echo $bsrt["wilayah"]; ?>' + '\n\n' + 'Jika YA silahkan klik OK, Jika TIDAK klik CANCEL.')"><button>HAPUS</button></a></td>
    </tr>
<?php
	$i++;
}

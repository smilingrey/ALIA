<script type="text/javascript">
	function Kirim_Cari()
	{
		...
		var s;
		s='./index.php?mod=home&opt=kotak&opts=list';
...
<?php
	...
	if(!empty($_GET["nama"])) 
	{
		$qsrt = mysql_query("select * from tbl_kotak where id_kotak is not null and ".$up."");
	}
	else
	{
		$qsrt = mysql_query("select * from tbl_kotak");
	}	
	$batas = 20;
	...
	if(!empty($_GET["nama"])) 
	{
		$qsrt = mysql_query("select * from tbl_kotak where id_kotak is not null and ".$up." limit ".$posisi.",".$batas."");
	}
	else
	{
		$qsrt = mysql_query("select * from tbl_kotak  limit ".$posisi.",".$batas."");
	}
	...
	echo '</span></form>';
?>
	<div><a href="./index.php?mod=home&opt=kotak&opts=tambah" class="link" title="Klik untuk tambah"><button>TAMBAH NOMOR KOTAK</button></a></div><br />
...
?>
	<tr id="tbl-content">
        ...
        <td align="center" valign="top"><a class="edit" href="./index.php?mod=home&opt=kotak&opts=edit&id_kotak=<?php echo $bsrt["id_kotak"]; ?>"><button>EDIT</button></a></td>
        <td align="center" valign="top"><a class="hapus" href="./index.php?mod=home&opt=kotak&opts=validasi&mode=3&id_kotak=<?php echo $bsrt["id_kotak"]; ?>" onClick="return confirm('Apakah Anda yakin ingin menghapus data ini?' + '\nNomor: ' + '<?php echo $bsrt["nama"]; ?>' + '\n\n' + 'Jika YA silahkan klik OK, Jika TIDAK klik CANCEL.')"><button>HAPUS</button></a></td>
    </tr>
<?php
	$i++;
}
